...
function acceptanceTest3(): void {
  ...
  const joueur = new Joueur(new FakeGuerrier("Arthur"), terrain, 0, 0);
  const message = joueur.processCommand("S"); 
  assert.ok(
...
function acceptanceTest8(): void {
  ...
  let message = joueur.processCommand("D"); 
  assert.ok(
    message.includes("E"),
    "Acceptance Test 8: Après 'D', l'orientation doit être E."
  );
  message = joueur.processCommand("G"); 
  assert.ok(
    message.includes("N"),
...
function acceptanceTest9(): void {
  ...
  const message = joueur.processCommand("A"); 
  assert.ok(
